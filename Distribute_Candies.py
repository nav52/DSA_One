...
class Solution(object):
    def distributeCandies(self, candyType):
        ...
        :rtype: int
        """
        num_of_candies = len(candyType)
        eligible_candies = num_of_candies//2
        
        unique_candies = set(candyType)
        
        if len(unique_candies) == eligible_candies:
            return eligible_candies
        elif len(unique_candies) < eligible_candies:
